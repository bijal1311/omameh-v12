...
export function wireInteractions(root) {
  ...
  // ── Reveal cards · tap-to-toggle (mobile) ──────────────────────
  // Home Group tiles, About Speciality Domains, and Practice Five Pillar
  // rows use .card--reveal / .pillar--reveal. On desktop the expanded body
  // shows on :hover / :focus-within (pure CSS). On touch devices the old
  // behaviour was to render everything expanded always — which read as a
  // layout bug ('shows in full'). We now wire tap-to-toggle: clicking the
  // card body toggles .is-open. Clicks on inner links/buttons pass through
  // (so the CTAs still work as links).
  root.querySelectorAll('.card--reveal, .pillar--reveal').forEach((card) => {
    const onClick = (e) => {
